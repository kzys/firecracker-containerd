...
func tryConnect(logger *logrus.Entry, udsPath string, port uint32) (net.Conn, error) {
	...
	if !strings.HasPrefix(string(line), "OK ") {
		return nil, vsockAckError{
			cause: errors.Errorf(`expected to read "OK <port>", but instead read %q`, string(line)),
		}
	}
	return conn, nil
}
...
// tryConnReadUntil will try to do a read from the provided conn till encounter
// the specified end character, returning an error if the read does not complete
...
func tryConnReadUntil(conn net.Conn, end byte, timeout time.Duration) ([]byte, error) {
